...
signal sig_message_received(peer_id: int, message: String)
signal sig_client_connected(peer_id: int)
signal sig_client_disconnected(peer_id: int)
...
const DEFAULT_CERT_PATH = "res://resources/tls/cert.pem"
const DEFAULT_KEY_PATH = "res://resources/tls/key.pem"

var _tls_loaded: bool = false


func init() -> void:
	print("WebSocketServer: Loading TLS certificates from ", DEFAULT_CERT_PATH, " and ", DEFAULT_KEY_PATH)
	_tls_loaded = load_tls_certificates(DEFAULT_CERT_PATH, DEFAULT_KEY_PATH)
	if not _tls_loaded:
		push_error("WebSocketServer: Failed to load TLS certificates")
	else:
		print("WebSocketServer: TLS certificates loaded successfully")
		print("  tls_cert: ", tls_cert)
		print("  tls_key: ", tls_key)
...
func poll() -> void:
	...
	for id: int in peers:
		...
		if p.get_ready_state() != WebSocketPeer.STATE_OPEN:
			sig_client_disconnected.emit(id)
			to_remove.append(id)
			continue

		while p.get_available_packet_count():
			sig_message_received.emit(id, get_message(id))
...
func _connect_pending(p: PendingPeer) -> bool:
	if p.ws != null:
		...
		if p.connection == p.tcp:
			if not _tls_loaded or tls_key == null or tls_cert == null:
				push_error("TLS certificates not loaded, cannot accept TLS connection")
				return true  # Failure
...
func load_tls_certificates(cert_path: String, key_path: String) -> bool:
	var cert_file := FileAccess.open(cert_path, FileAccess.READ)
	if not cert_file:
		push_error("Failed to load TLS certificate: " + cert_path)
		return false

	var key_file := FileAccess.open(key_path, FileAccess.READ)
	if not key_file:
		push_error("Failed to load TLS private key: " + key_path)
		return false

	tls_cert = X509Certificate.new()
	tls_cert.load(cert_path)
